    ...
    def __init__(self, X_h_IC, 
        ...
        if ExistModel == 0:
            ...
            if self.useObs:
                ...
                self.adaptive_constant_bcs_u_tf, \
                self.adaptive_constant_bcs_h_tf, \
                self.adaptive_constant_ics_h_tf = self.load_weight(wDir)
                print("constant_bcs_u_val: {:.3f}, constant_bcs_h_val: {:.3f}, constant_ics_h_val: {:.3f}".format(self.adaptive_constant_bcs_u_tf, self.adaptive_constant_bcs_h_tf, self.adaptive_constant_ics_h_tf))
        ...
        self.dict_gradients_bcs_u_layers = self.generate_grad_dict(self.layers)
        self.dict_gradients_bcs_h_layers = self.generate_grad_dict(self.layers)
        self.dict_gradients_ics_h_layers = self.generate_grad_dict(self.layers)
        if self.useObs:
            self.dict_gradients_obs_u_layers = self.generate_grad_dict(self.layers)
            self.dict_gradients_obs_h_layers = self.generate_grad_dict(self.layers)
